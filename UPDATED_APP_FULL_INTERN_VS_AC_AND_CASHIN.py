...
def _render_marketing_intern_vs_ac(df):
    ...
    Uses ONLY event dates for counts:
    ...
    Features:
      - Two comparison windows (A & B) with presets & custom range.
      - Segment type filter: Intern / AC (multi-select).
      - Name-level filters:
          * Intern(s) multi-select (or All)
          * AC name(s) multi-select (or All)
      - Breakdown by:
          * Overall
          * Country
          * JetLearn Deal Source
    ...
    Output table always matches chosen Breakdown.
    """
    import streamlit as st
    import pandas as pd
    from datetime import date, timedelta
    ...
    if "Intern" in selected_segs and intern_col:
        intern_names = sorted(df.loc[seg == "Intern", intern_col].dropna().astype(str).str.strip().unique())
        with col_intern:
    ...
    if "AC" in selected_segs and ac_col:
        ac_names = sorted(df.loc[seg == "AC", ac_col].dropna().astype(str).str.strip().unique())
        with col_ac:
    ...
    def _metrics_for(seg_name: str, dim_val, start, end):
        ...
        if seg_name == "Intern" and intern_col and selected_intern_names and "All" not in selected_intern_names:
            base_mask &= df[intern_col].astype(str).str.strip().isin(selected_intern_names)

        if seg_name == "AC" and ac_col and selected_ac_names and "All" not in selected_ac_names:
    ...
    if not summary_df.empty:
        try:
            ...
            chart = alt.Chart(summary_df).mark_bar().encode(
                ...
                y=alt.Y("First Cal:Q", title="First Cal"),
...
# ================================
# Performance ▶ Cash-in (Google Sheet: A1:BB13)
# ================================
def _render_performance_cash_in():
    """Cash-in view powered by the specified Google Sheet.

    Sheet:
      https://docs.google.com/spreadsheets/d/1ygmNCuERA2kkWGztQCmX9sTZlyTZcLp_UM0_5GPZxN4/edit?gid=0#gid=0

    Logic:
      - Load range A1:BB13 (gid=0) as the Cash-in summary table.
      - Show as a single table under the Cash-in pill.
    """
    import streamlit as st
    import pandas as pd
    ...
    try:
        ...
        st.error(f"Unable to load Cash-in data from Google Sheet (A1:BB13): {e}")
        return
